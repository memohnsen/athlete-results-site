...
export default function AthleteTable() {
    ...
    async function fetchData() {
        ...
        try {
            ...
            const { data, error } = await supabase.from("lifting_results").select().limit(itemsPerPage) 
    ...
    return (
        <main className="flex flex-col mx-auto items-center mb-10">
            <div className='justify-between'>
                <input type='search' placeholder=" Athlete Name" className="border p-1" />
                <input type='select' placeholder="Meet" className="border p-1" />
                <form>
                    <label>Rows Per Page: </label>
                    <select name="rows" onChange={selectRows} value={itemsPerPage}>
